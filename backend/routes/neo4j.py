...
from backend.services.neo4j_client import check_connectivity, get_read_session
...
def neo4j_flag_account(account_id: str):
    ...
    # Neo4j writes are disabled per policy; only local flag stored.
    return jsonify({"status": "ok", "accountId": account_id, "neo4j_write": "skipped"})


@neo4j_bp.route("/neo4j/flag/device/<device_id>", methods=["POST"])
def neo4j_flag_device(device_id: str):
    _record_flag(device_id, "DEVICE")
    return jsonify({"status": "ok", "deviceId": device_id, "neo4j_write": "skipped"})
